...
export const LESSONS = ['move','attack','tower','crystal','deposit','upgrade'];
...
export function readOpening(storage) {
  try {
    ...
    return {intro:value.intro===true,story:value.story===true,skipped:value.skipped===true,
      done:LESSONS.filter(id=>Array.isArray(value.done)&&value.done.includes(id)),
...
export function holdsFirstWave(state) { return !state.skipped&&['move','attack','tower'].some(id=>!state.done.includes(id)); }
