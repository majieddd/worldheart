...
export function updateHandling(s,dt,{x=0,z=0,yaw=0,run=false,crouch=false,aim=false,attack=false,grounded=true,flight=false,blocked=false,pace=HANDLING}={}){
  ...
  const start=!flight&&grounded&&crouch&&!s.previousCrouch&&run&&z<-.4&&speed>4.5*pace.sprint/HANDLING.sprint&&!s.cooldown&&!s.slide;
  if(start){s.slide=HANDLING.slideDuration;s.slideSpeed=pace.slide;s.slideX=s.vx/speed;s.slideZ=s.vz/speed;}
  ...
  if(s.slide){
    ...
    const targetSpeed=flight?(run?12:6):crouch?pace.crouch:aim?pace.aim:sprint?pace.sprint:pace.walk;
    const response=flight?8:!grounded?5.5:Math.hypot(x,z)>.01?30:38;
    s.vx=damp(s.vx,wx*targetSpeed,response,dt);s.vz=damp(s.vz,wz*targetSpeed,response,dt);
  }
