...
export async function createVeyActor(paint){
  const gltf=await new GLTFLoader().loadAsync('lib/99-art/vey-articulation-v1/vey-motion.glb');
  ...
  let current='Idle';
  return {root,model,hand,meshes,mixer,clips,groundSpeeds:{Walking:VEY_PACE.walk,Running:VEY_PACE.sprint},
    ...
    action(){return false;},
    tick(dt,speed){
      const moving=THREE.MathUtils.smoothstep(speed,.015,.35),running=THREE.MathUtils.smoothstep(speed,1.6,2.8);
      const weights={Idle:1-moving,Walk:moving*(1-running),Run:moving*running};
      current=moving<.1?'Idle':running>.5?'Run':'Walk';
      for(const [name,a]of Object.entries(actions)){a.setEffectiveWeight(THREE.MathUtils.damp(a.getEffectiveWeight(),weights[name]||0,14,dt));a.timeScale=name==='Idle'?1:Math.max(.12,speed/(name==='Walk'?VEY_PACE.walk:VEY_PACE.sprint));}
