...
export class WeaponPanel {
  constructor({ game, possession, ui, api, rules }) {
    ...
    addEventListener('keydown', e => {
      ...
      if (e.code === 'KeyX' && !this.dialog.open && api.canInteract()) {
        e.preventDefault(); const slot = api.inventory.active === 0 ? 1 : 0;
        api.request({kind:'select',slot:api.inventory.slots[slot] ? slot : 'native'});
      }
      if (e.code === 'KeyR' && !this.dialog.open && api.canInteract()) { e.preventDefault(); const first = api.nearby()[0]; if (first && !api.pickup(first.id)) this.open(); }
    });
  }
  open() {
  ...
  render() {
    ...
    const equipped = new Set(inv.slots.filter(Boolean));
    ...
    const card = (item, ground = false) => {
      ...
      return `<article class="weapon-item rarity-${item.rarity}"><div class="weapon-item-heading"><strong>${name(item)}</strong><span>${item.rarity} · tier ${item.tier}</span></div><p>${metrics}</p><p class="weapon-parts">${Object.keys(parts).map(k=>parts[k][item.parts[k]].name).join(' / ')}${item.affixes.length ? ' · '+item.affixes.join(', ') : ''}</p>
    ...
    this.dialog.innerHTML = `<header><div><small>COMMANDER LOADOUT</small><h2>Weapons</h2><p>${trait}</p></div><button data-action="close">Resume</button></header>
      ...
      <p class="weapon-footnote">Compatible parts are free to swap in this prototype. Salvaging is permanent for this assault. Extraction and checkpoint banking arrive with campaign continuation.</p>`;
  ...
  update() {
    ...
    this.cue.textContent = near.length ? `R · ${this.rules.name(near[0])} · ${near[0].rarity}${this.api.inventory.items.length - this.api.inventory.slots.filter(Boolean).length >= 12 ? ' · Backpack full: I to replace' : ''}` : active ? `${this.rules.name(active)} · X switch · I parts` : '';
