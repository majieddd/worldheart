...
function fixture(t) {
  ...
  const repo = name => {
    ...
    const files = {
      'index.html': name, 'debug.html': name, 'style-lab.html': name, 'painted-lab.html': name, 'hard-cel-lab.html': name, 'art-candidates/hard-cel-v1/candidate.json': name, 'js/main.js': name, 'css/style.css': name,
...
test('Pages preserves production bytes and replaces only the v2 game with preview assets', t => {
  ...
  assert.equal(readFileSync(join(out, 'v2/hard-cel-lab.html'), 'utf8'), 'preview');
  ...
  assert.equal(Object.keys(build.previewHashes).length, 9);
});
