...
try{
 ...
 results.push(...await page.evaluate(async()=>{
  ...
  const weapons=await import(new URL('js/run/weapons.js',location.href)),item=weapons.generateWeapon({id:'expedition-salvage',seed:52,tier:1,family:'sword',rng:()=>.1}),scrapBefore=m.inventory.scrap;m.inventory.register(item);m.inventory.pickup(item.id);
  ck('A weapon salvages once into forging scraps',m.weapons.salvage(item.id)&&!m.weapons.salvage(item.id)&&m.inventory.scrap===scrapBefore+1);ck('Orange pickups no longer appear',!m.oreField&&!m.ore&&m.forge.balance===m.inventory.scrap);
