...
try{
 ...
 await page.goto('http://127.0.0.1:8141/?map=ninetynine&campaign=1&seed=12345',{timeout:120000});
 ...
 await page.evaluate(async()=>{
  ...
  const W=WH,{surfacePoint}=await import('/js/world.js');window.__a=W.allies.active[0];
 ...
 await page.evaluate(async()=>{
  const W=WH,a=__a,m=W.mode99,{generateWeapon}=await import('/js/run/weapons.js'),{makeRng}=await import('/js/run/rng.js');
 ...
 await page.evaluate(async()=>{
  const m=WH.mode99,{generateWeapon}=await import('/js/run/weapons.js'),{makeRng}=await import('/js/run/rng.js');
 ...
 writeFileSync(resolve(out,'results.json'),JSON.stringify({scope:'Instrumented campaign checkpoint, actual pointer/keyboard/UI and inventory edge cases',checks,faults},null,2)+'\n');
