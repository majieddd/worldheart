...
log=Path(a.log).resolve();log.parent.mkdir(parents=True,exist_ok=True);start=time.perf_counter()
receipt=log.with_suffix('.timing.json')
report={'startedAt':datetime.now(timezone.utc).isoformat(),'status':'running','fullLog':str(log)}
...
try:
    ...
    report.update(exitCode=result.returncode,status='passed' if result.returncode==0 else 'failed')
except BaseException as e:
    report.update(status='failed',error=str(e));raise
finally:
    report.update(endedAt=datetime.now(timezone.utc).isoformat(),seconds=round(time.perf_counter()-start,2))
    receipt.write_text(json.dumps(report,indent=2),encoding='utf-8')
