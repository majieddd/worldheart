...
def identity(hero,description,style,settings):
 scripts={name:sha(Path(__file__).with_name(name))for name in ['reference_pack.py','workflows.py','server.py']}
...
def prompts(description,style):
 base=f'{description}. {STYLE[style][1]} The FIRST reference is the exact character identity. Preserve every costume piece, face, proportions, colors and asymmetry. '
 result={name:base+f'Production turnaround: ONE character, {view}. Neutral relaxed A-pose identical to the reference, complete silhouette with generous margins, orthographic-like camera, even neutral light, plain light gray background. No scenery, no extra figures, no labels, no shadow on the background.' for name,view in VIEWS.items()}
...
def assemble(root,pack,description):
 ...
 d.text((24,1550),'SAMPLED sRGB PALETTE / verify material roles before painting',font=small,fill='#243830')
