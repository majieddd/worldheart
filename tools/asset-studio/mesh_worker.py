...
p=argparse.ArgumentParser();p.add_argument('--runtime',required=True);p.add_argument('--image',required=True);p.add_argument('--output',required=True);p.add_argument('--steps',type=int,default=30);p.add_argument('--resolution',type=int,default=256);p.add_argument('--seed',type=int,default=99131);a=p.parse_args()
...
image.save(str(Path(a.output).with_suffix('.input.png')))
start=time.time();print('Loading local Hunyuan3D mini',flush=True)
pipeline=Hunyuan3DDiTFlowMatchingPipeline.from_pretrained(str(runtime/'models/Hunyuan3D-2mini'),subfolder='hunyuan3d-dit-v2-mini',variant='fp16',device='cpu')
...
Path(a.output).with_suffix('.json').write_text(json.dumps({'method':'local Hunyuan3D-2mini image-to-shape','input':a.image,'seed':a.seed,'steps':a.steps,'resolution':a.resolution,'vertices':len(mesh.vertices),'faces':len(mesh.faces),'seconds':time.time()-start,'textUse':'The approved image resolves the written brief; geometry is image-conditioned.','acceptance':'Unreviewed reconstruction; inspect anatomy and rear surfaces.'},indent=2),encoding='utf-8')
