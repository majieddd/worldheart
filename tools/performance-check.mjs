...
import{createRequire}from'node:module';import{resolve}from'node:path';import{mkdirSync,writeFileSync,readFileSync}from'node:fs';import{cpus,totalmem,platform,release}from'node:os';
...
const viewport=mobile?{width:844,height:390}:process.argv.includes('--compact')?{width:1280,height:720}:{width:1920,height:1080};
const browser=process.env.WH_BROWSER_WS?await chromium.connect(process.env.WH_BROWSER_WS):await chromium.launch({channel:'chrome',headless:true}),page=await browser.newPage({viewport,deviceScaleFactor:mobile?3:1,isMobile:mobile,hasTouch:mobile}),faults=[];
if(baseline)for(const file of ['nav.js','terrain/features.js','terrain/active-features.js','world-context.js'])await page.route('**/js/'+file,route=>route.fulfill({contentType:'text/javascript',body:readFileSync(resolve(baseline,file),'utf8')}));
...
let cdp;
try{
 cdp=await page.context().newCDPSession(page);await cdp.send('Emulation.setCPUThrottlingRate',{rate:1});
 ...
 report.viewport.dpr=mobile?3:1;report.baseline=baseline||null;
 report.uploadPolicy=fullUploads?'Full-buffer ablation; ignores live-prefix ranges':'Runtime default';
 report.setup=await page.evaluate(async ({fullUploads,baseline})=>{
   ...
   if(baseline){if(W.world.terrainChunks)W.world.terrainChunks.forceFull=true;for(const s of W.world.decor.sets)if(s.sectors)s.sectors.forceFull=true;}
   ...
   return{seed:W.CONFIG.seed,terrain:W.CONFIG.terrainKey,nodes:W.nav.n,heuristic:W.nav._routeHeuristicScale,towers:W.towers.towers.length,enemies:W.enemies.active.length,heart:W.game.lives,commander:W.allies.active.filter(a=>a.type.commander).map(a=>a.hp),failures,gpu:debug?gl.getParameter(debug.UNMASKED_RENDERER_WEBGL):gl.getParameter(gl.RENDERER)};
 },{fullUploads,baseline});
 await page.waitForTimeout(10000);
 // Fix both sides to the owner's full visual settings after auto-detection.
 // Slow only the measurement, so startup and fixture preparation do not
 // change the workload. This is a CPU proxy, never a simulated mobile GPU.
 await page.evaluate(()=>WH.ui.onQuality('high'));
 ...
 // Screenshot readback/encoding can itself stall Chrome for hundreds of
 // milliseconds. Retain an image only after sampling stops, not inside the
 // gameplay frame distribution that is meant to measure the game.
 await page.screenshot({path:resolve(out,'stress-complete.png')});
 if(profiling){
...
}catch(error){report.error=String(error);process.exitCode=1;}finally{if(cdp)await cdp.send('Emulation.setCPUThrottlingRate',{rate:1});writeFileSync(resolve(out,'results.json'),JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify({...report,measurement:report.measurement?{...report.measurement,loads:report.measurement.loads.length}:null}));await browser.close();}
