...
if(!profile){
 ...
 writeFileSync(resolve(out,'results.json'),JSON.stringify({scope:'Current terrain and blocked graph comparison with d79f1d6 full placement/preview searches; real directed costs and unchanged authoritative fields',results,pass:results.every(r=>r.pass)},null,2)+'\n');if(results.some(r=>!r.pass))process.exitCode=1;
}else{
 ...
 function probe(node,radius,label){
  ...
  const oracle=oldPreview.call(n,pos,radius),before={next:n.next.slice(),dist:n.dist.slice(),air:n.airNext?.slice(),block:n.block.slice(),revision:n.revision};
  ...
  const unchanged=same(before.next,n.next)&&same(before.dist,n.dist)&&same(before.block,n.block)&&(!before.air||same(before.air,n.airNext))&&before.revision===n.revision;
