...
try{
  ...
  await page.locator('#home-claim').click();await page.waitForURL('**home=home-*',{timeout:15000});await ready();await page.locator('#btn-begin').click();
  ...
  await page.locator('#home-cancel-place').click();
  ...
  await page.evaluate(()=>WH.step(12));check('Home incursion establishes a physical nest',await page.evaluate(()=>WH.mode99.home.running&&WH.waves.wave===1&&WH.world.portals.some(p=>p.active&&p.established)));
  ...
  for(let i=0;i<140;i++){
    const done=await page.evaluate(()=>{WH.step(1);for(const e of [...WH.enemies.active])WH.enemies.damage(e,1e9,{armorPierce:99});return WH.mode99.home.quiet&&WH.mode99.home.record.checkpoint.run.wavesCleared===1;});
    if(done)break;
  }
  check('Stopping finishes and checkpoints a cleared wave',await page.evaluate(()=>WH.mode99.home.quiet&&WH.mode99.home.record.checkpoint.run.wavesCleared===1));
  ...
  await page.reload();await ready();await page.locator('#btn-begin').click();
  check('Wave checkpoint persists across reload',await page.evaluate(()=>WH.mode99.run.getWave()===2&&WH.mode99.home.quiet));
  ...
  await phone.goto(new URL(homeURL,base+'/lobby.html').href);await phone.waitForFunction(()=>window.WH?.mode99&&document.querySelector('#boot.done'),null,{timeout:180000});await phone.locator('#btn-begin').tap();
