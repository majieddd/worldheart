...
for(const p of factions.plates){const f=factions.factions.find(f=>f.id===p.faction);add(p,root+'/factions','Factions',f.name+' / '+p.type,p.faction==='alien'?'Earlier Physical-material study; current elemental canon is in the Xeno codex.':'Commanders, equipment and homeworld proposals.','Catalogue study');}
