...
try{
 ...
 check('Planet theme owns climate without a duplicate selector',await lab.locator('#worldgen-biome').count()===0&&await lab.locator('#worldgen-planet').count()===1);
 await lab.locator('#worldgen-paths').check();await lab.evaluate(()=>WH.worldgen.routeView.routesReady);check('Real nest approach overlay reaches the heart from seventeen example approaches (independent of the wave cap)',await lab.evaluate(()=>WH.worldgen.routeCount===17&&WH.worldgen.paths.visible));
