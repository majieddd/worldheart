...
try{for(const variant of variants){
 ...
 await page.evaluate(async()=>{
  __qaFramesEnabled=false;WH.game.paused=true;WH.step(.01);const W=WH,g=W.game,{surfacePoint}=await import(new URL('js/world.js',location.href));
  const centers=[],dir=g.cursorDir.clone(),center=W.heartPos.clone().normalize(),bearing=W.allies.active[0].fwd.clone();
  for(let i=0;i<240;i++){dir.copy(center).addScaledVector(bearing,(4+Math.sin(i/30)*2)/240).normalize();centers.push(surfacePoint(dir,dir.clone()));}
  window.__guideCenters=centers;g.buildType='bolt';g.cursorValid=true;g.cursorDir.copy(centers[0]).normalize();g.cursorPos.copy(centers[0]);g._mountGhost('bolt');
 });
 ...
 for(const cpuRate of (process.argv.includes('--diagnose')?[1]:[1,4])){
  ...
  const data=await page.evaluate(async()=>{
   ...
   for(let i=0;i<280;i++){
    ...
    const p=__guideCenters[i%240];g.cursorPos.copy(p);g.cursorDir.copy(p).normalize();const start=performance.now();g._updateGhost();WH.step(1/60);if(i>=40)work.push(performance.now()-start);
   ...
   return {seed:WH.CONFIG.seed,framesMs:stats(frames),mainWorkMs:stats(work),guideMs:stats(guide.slice(40)),operations:Object.fromEntries(Object.entries(operations).map(([k,v])=>[k,stats(v)])),geometryIds:geometry.size,visible:g.rangeRing.mesh.visible};
