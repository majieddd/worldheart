U272: Space jumps, F interacts, and the first tutorial retains cursor capture. Locked crosshairs can highlight and activate contextual buttons. Walls share the tower hotbar (J on desktop); their route fields update incrementally. A 5 m emplacement gives 20% extra horizontal range, capped at +100% at 25 m. Prior UI fonts/colors restored; simplified weapon cards remain.

U262-U271, September 19: terrain version 3 gives Solar bodies fixed geography, 105 tactical landmarks and denser region coverage while preserving old saved surfaces. Bridges share picking/support/navigation layers. New active features are 1.6x; eight building families eject one-time supplies with animation and sound. Walls snap on the globe and rotate by wheel; mobile shares tower/wall cards and directional pinch. A large commander courtyard contains seven stations and two join pads; three scraps roll a weapon and three matching unequipped weapons merge up one rarity, capped at rare at home. Painted weapon cards emphasize three stats and expandable detail. Laya recipe classification remains an offline experiment after failing to beat simple rules. [Contracts and evidence](qa/implementation/WORLD-EXPLORATION-POLISH.md).

# 99 Planets To Defend: running development blueprint

U256-U261, September 19: the production game adopts the approved Painted-Anime-Inkline 1.3.2 surface recipe while retaining its instanced rigs and authored terrain. Ordinary entry begins or resumes the Earth-first campaign; explicit home/debug links keep their destination. Revised illustrated lore closes into FPS. An action-led build, upgrade and crystal tutorial holds the first countdown; later briefs introduce combat, powers, salvage, strategy, bosses and claiming. Five 180-HP wooden walls cost one scrap and permit detours or breaches. Four explorable structure recipes grant one-time seeded supplies and appear in Debug World. New Solar worlds use terrain version 2 for moderate relief exaggeration; old saves keep version 0/1 geometry. [Implementation status and measured evidence](qa/implementation/PAINTED-EARTH.md).
...
Published at V2 `0e8723e`, PR #55, with 48 public browser and 261 identity checks.
...
cannon fire and illustrative feedback in both treatments. Published and publicly
verified at V2 060c78c; [publication tracking](qa/implementation/STYLE-PLAYGROUND.md).
