...
`audio-catalogue.js` owns semantic cues, buses, priorities and bounded settings;
`audio.js` owns one decoded effects bank and two streamed music decks. Gameplay
callbacks emit cues without owning sound graphs. UI and Debug share
`audio-settings.js`; storage uses the existing route-scoped adapter.
`audio-bank.js` and `audio-music.js` contain delivery metadata, not simulation.
Local YuE2 and sound-design tools live in `tools/audio/`. Track generation,
mastering, rights and hashes are documented in `audio/music-provenance.json`.
Use the [audio ledger](qa/implementation/AUDIO-REVAMP.md) for evidence boundaries.
