...
|---|---|---|---|---|
| N1 Rounded landform regions | verified locally; publication pending | world terrain field, coherent foothills and retained height; fixed-seed field comparisons and rendered inspection | 36 shape assertions, four profile fixtures, all 99 isolated routes | unmeasured |
| N2 Obstacle and camera response | verified locally; publication pending | actual decor bounds replace fixed-height camera obstacles; real terrain/tower movement barriers retained | 15 decor probes after vs 6 before; ten current-layout surface/path checks pass at 30/60/120 Hz | unmeasured |
| N3 First-person motion and text | targeted fixes verified locally; full motion review open | actual displacement drives bob; reset residual sway, deduplicate warnings and anchor reduced-motion text | 7 motion/text assertions after vs 0 before, 16 possession-camera and 12 held-prop checks | unmeasured |
| N4 Integrated acceptance and publication | verification complete for this checkpoint; publication active | route/cost repairs, full legal planet-1 run, core/combat regression, measured performance, V2 identity and public behavior | 228 core tests, 1,485 route arrivals, 486 physical-nest arrivals, 180 path comparisons and actual victory/extraction; reduced-CPU gate open | unmeasured |
...
Publication and final combined measurements will be recorded below after the
verified checkpoint is deployed. Main is not a target of this batch.
