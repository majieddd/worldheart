...
export function installAssaultPolicy(W, trace) {
  ...
  return {
    ...
    update(dt) {
      ...
      if (unit.hp < unit.hpMax * .45) resting = true;
      if (unit.hp > unit.hpMax * .85) resting = false;
      ...
      if (resting) {
        setGoal('recover-at-heart', W.nav.fieldCenter, 'recover');
      } else if (hunting) {
      ...
      if ((target.kind === 'nest' && nestDistance < (spec.radius || 3) + 2) || inMelee) {
      ...
      const turn = aimAt(unit, goalWorld); key('KeyW', Math.abs(turn) < .6); key('ShiftLeft', Math.abs(turn) < .25 && !unit.swimming && !(danger?.distance < 7));
      if (stalled > 2) {
