...
try{manifest=await fetch(base+'manifest.json').then(r=>{if(!r.ok)throw Error('Candidate manifest unavailable');return r.json();});await preview.load(base+manifest.model,e=>{$('#status').textContent='Loading commander / '+(e.total?Math.round(e.loaded/e.total*100)+'%':(e.loaded/1048576).toFixed(1)+' MB');});preview.yaw=.35;preview.pitch=.07;preview.distance=preview.height*2.35;clip('Idle');preview.pauseRendering(false);$('#reference').src=base+manifest.hero;$('#download').href=base+manifest.model;$('#status').textContent=manifest.status;controls.forEach(el=>el.disabled=false);$('#viewer').setAttribute('aria-busy','false');window.COMMANDER_PREVIEW.ready=true;window.COMMANDER_PREVIEW.manifest=manifest;}catch(error){$('#status').textContent=error.message;window.COMMANDER_PREVIEW.error=error.message;}
