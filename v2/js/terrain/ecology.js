...
export function createEcology(seed, key='auto',environment=planetEnvironment((seed>>>0)||1)) {
  ...
  const temperature=(x,y,z,height=0)=>.52-Math.pow(Math.abs(y),1.15)*1.12+heat(x*3.2+43,y*3.2,z*3.2)*.1+warmth-Math.max(0,height)*.006;
  ...
  return {temperature,moisture,biome,volcanic,
    ...
    latitude,
    manifest(){return {version:3,seed,key,regime:regime.name,warmth,wetness,environment,bands:['Wet equator','Dry subtropics','Temperate woodlands','Polar tundra']};},
  };
}
