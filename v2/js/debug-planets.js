...
export function terrainCollection(key,seed=MINIATURE_SEED){
  ...
  try{
    ...
    for(const candidate of modules){const c=new THREE.Vector3(...candidate.dir);if(Math.abs(c.y)>.3)continue;const nearby=modules.filter(m=>c.dot(new THREE.Vector3(...m.dir))>Math.cos(.55));const n=new Set(nearby.map(m=>m.type)).size;
