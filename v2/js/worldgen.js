...
export function worldgenUrl(base, seed, terrain = 'varied', inspect = true, biome = 'auto',planet = 'auto') {
  ...
  if (![...Object.keys(TERRAIN_PACKS),'classic'].includes(terrain)) throw Error('Choose a supported terrain.');
...
export function rememberWorld(history, world) {
  const valid = x => x && Number.isInteger(x.seed) && x.seed > 0 && x.seed <= 0xffffffff
    && [...Object.keys(TERRAIN_PACKS),'classic'].includes(x.terrain) && (!x.biome || ['auto','temperate','desert','boreal','jungle','volcanic','wetland'].includes(x.biome)) && (!x.planet || Object.hasOwn(PLANET_THEMES,x.planet));
