...
const rest={p:[.34,-.48,-.66],r:[-.73,.1,-.25]};
// Authored key poses, in camera space: guard / wind-up / cutting path / follow-through / recovery.
const cuts=[
  [[.0,rest.p,rest.r],[.22,[.48,-.24,-.46],[-.45,.4,-1.05]],[.43,[-.20,-.29,-.94],[-1.45,-.45,.55]],[.65,[-.34,-.62,-.62],[-1.9,-.35,1.1]],[1,rest.p,rest.r]],
  [[0,rest.p,rest.r],[.22,[-.31,-.22,-.5],[-.25,-.5,.9]],[.44,[.33,-.25,-.96],[-1.4,.4,-.75]],[.65,[.53,-.55,-.55],[-1.9,.65,-1.25]],[1,rest.p,rest.r]],
  [[0,rest.p,rest.r],[.27,[.12,.05,-.42],[.6,.05,-.1]],[.48,[.03,-.42,-1.02],[-1.5,0,.1]],[.69,[.08,-.70,-.62],[-2.0,.1,.2]],[1,rest.p,rest.r]]
];
function sample(keys,t){let a=keys[0],b=keys.at(-1);for(let i=1;i<keys.length;i++)if(t<=keys[i][0]){a=keys[i-1];b=keys[i];break;}const u=smooth(THREE.MathUtils.clamp((t-a[0])/(b[0]-a[0]||1),0,1));return {p:v3(a[1]).lerp(v3(b[1]),u),q:quat(a[2]).slerp(quat(b[2]),u)};}

export function createWeapons(paint){
  const k=kit(paint),root=new THREE.Group(),sword=k.weapon('sword'),rifle=k.weapon('rifle');root.add(sword.root,rifle.root);sword.root.scale.setScalar(.82);rifle.root.scale.setScalar(.72);
  const arms=[];
  ...
  function update(dt,{weapon='sword',attack=0,combo=0,moving=0,aim=false,vent=0,bob=.35,look=[0,0],time=0}={}){
    ...
    sword.root.visible=weapon==='sword';rifle.root.visible=weapon==='rifle';let pose=weapon==='sword'&&attack>0?sample(cuts[combo%3],attack):{p:v3(weapon==='sword'?rest.p:[aim?.04:.32,aim?-.34:-.43,-.61]),q:quat(weapon==='sword'?rest.r:[kick*.13,0,0])};
    ...
    held.root.updateMatrixWorld(true);const support=weapon==='rifle'?held.root.localToWorld(v3([-.1,-.02,-.44])):v3([-.35,-.53,-.5]);arm(arms[0],support,quat(weapon==='rifle'?[-.1,0,-.15]:[.1,0,.2]));for(const a of arms)for(const f of a.fingers)f.rotation.x=weapon==='sword'?.2:.42;
