...
export function createWeapons(paint){
  ...
  let kick=0,lookX=0,lookY=0,phase=0,lastWeapon='sword',equip=1,aimBlend=0;const tip=new THREE.Vector3(),base=new THREE.Vector3();
  ...
  function update(dt,{weapon='sword',attack=0,combo=0,moving=0,aim=false,ads=null,sprint=0,slide=0,vent=0,bob=.35,look=[0,0],time=0}={}){
    ...
    aimBlend=ads===null?THREE.MathUtils.clamp(aimBlend+(aim?1:-1)*dt/.2,0,1):ads;const blend=smooth(aimBlend),weight=1-blend*.97,pace=Math.min(moving,4),sprintWeight=sprint*(1-blend)*(attack?0:1);
    sword.root.visible=weapon==='sword';rifle.root.visible=weapon==='rifle';let pose=weapon==='sword'&&attack>0?(()=>{const p=swordPose(combo,attack);return {p:v3(p.p),q:quat(p.r)};})():{p:weapon==='sword'?v3(rest.p):v3([.32,-.43,-.61]).lerp(v3([0,-.324,-.61]),blend),q:quat(weapon==='sword'?rest.r:[kick*.075,0,0])};
    if(weapon==='sword'&&!attack){pose.p.lerp(v3([.03,-.3,-.73]),blend);pose.q.slerp(quat([-.25,-.2,1.2]),blend);}
    if(innerWidth/innerHeight<.8){pose.p.x*=.65;pose.p.z-=.15*(1-blend);}pose.p.x+=(lookX+Math.sin(phase)*.009*bob*pace)*weight;pose.p.y+=(lookY+Math.abs(Math.cos(phase))*.013*bob*pace)*weight-(1-smooth(equip))*.48;pose.p.z+=kick*.065;
    pose.p.y-=sprintWeight*.10;pose.p.x-=sprintWeight*.09;pose.q.multiply(quat([sprintWeight*.22,0,-sprintWeight*.23-slide*.045*(1-blend)]));if(vent>0){pose.p.y-=Math.sin(vent*Math.PI)*.2;pose.q.multiply(quat([0,0,-Math.sin(vent*Math.PI)*.65]));}
